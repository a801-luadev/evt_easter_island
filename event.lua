...
local spawnMiceCage = function(cagePosition)
	local x, y, w, h = cagePosition.x, cagePosition.y, cagePosition.w, cagePosition.h
	local hW, hH = w / 2, h / 2
	local qW, qH = w / 4, h / 4

	coverMapContent(x - hW - 30, y - hH - 30, w + 60, h + 60, '!')

	local ground = groundProperties.miceCage
	local tmpX

	for g = 1, 4 do
		tmpX = (g % 3 - 1)

		ground.width = tmpX == 0 and hW or 10
		ground.height = tmpX ~= 0 and hH or 10

		tfm.exec.addPhysicObject(700 + g, x - tmpX*qW, y - (((g % 2) + 2 - g) / 2)*qH, ground)
	end
end
...
local selectEggSetToOffer = function(cachedEggs)
	...
	-- If has completed none or all sets
...
local offerEgg = function(playerName, rarityType)
	...
	if targetSet[egg] then
		if rarityType == eggRarityType.rare then
			local furType = math_random(1, 100) < 70 and consumables.aliceRabbitFur
				or consumables.sansaoFur
...
eventPlayerDied = function(playerName)
	...
	-- HOTFIX
	tfm.exec.respawnPlayer(playerName)
	tfm.exec.movePlayer(playerName, cagePosition.x, cagePosition.y)
